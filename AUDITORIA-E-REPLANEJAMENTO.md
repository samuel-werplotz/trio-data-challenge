...
| C3 | ≥1 Dictionary + uso em query + por que não JOIN | `dict_institutions` + `REPORT.md` | **OK** | Fechado na 16, **remedido na 17.5**: lookup por linha 4 ms (dictGet) × 15 ms (JOIN) = **3,75×**; `GROUP BY` 10M **13,4×**; 90 dias **4,0×**. Os números da 16 (2,9× e 2 empates) foram medidos com o Dictionary resolvendo só 33,55% — ver desvio da 17.5 |
