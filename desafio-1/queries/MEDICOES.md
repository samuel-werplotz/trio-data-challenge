...
Protocolo de medição: cada query roda 4 vezes; a 1ª execução é descartada
...
| Q4 — duplicatas 5min, self-join (anti-padrão) | 2.814 ms | shared hit=baixo, read=499 | Mais rápida do que o anti-padrão faria esperar — o planejador escolheu **Parallel Hash Join**, não Nested Loop ingênuo. Ainda assim é o anti-padrão: plano varre e cruza ~959k linhas em 7 dias (5x o volume de referência do desenho, ~190k) sem aproveitar ordenação alguma. Comparar com a versão window function (etapa 07) deve mostrar ganho por eliminar a comparação todos-contra-todos, não necessariamente por tempo de parede nesta escala. |
...
O protocolo pede sempre reportar a comparação hit/read — não que ambos sejam > 0. Q1
