...
## ESTADO HERDADO
Verificado ao fechar a **etapa 21** (a última antes desta):
- **`run_all.sh`: 256 pass / 0 fail / 3 skip.** `audit.sh`: 83 pass / 0 fail / 2 warn / 1 skip. Os 3 SKIP são `make` ausente **neste** Windows — não são falha, e o passo 7 desta etapa precisa dizer isso por escrito.
- **A raiz tem só o `README.md`.** Processo consolidado em `docs/processo/` (5 arquivos) e o orquestrador virou `docs/METODO-DE-EXECUCAO.md`, com preâmbulo que assume a metodologia. **Ao escrever o README final (passo 4), partir do que já está lá** — a seção "Os 6 documentos que importam" e a de rastro de processo já existem.
- **Branch de entrega é `main`**, com 30 commits. Sem remote. `01.4` e `A7.7` já asseriam isso.
- **Duas etapas nasceram de defeito real achado ao medir**: 17.5 (Dictionary resolvendo 33,55%; a API servia `"desconhecida"` para 66% das instituições) e 20.5 (pipeline perdendo dados em silêncio acima de `BATCH_MAX_ROWS`). **É o material mais forte da apresentação** e já está no roteiro (`docs/HA-E-ROTEIRO-DEMO.md` § Parte 2, resposta 3).
- **As 5 perguntas do PDF já estão desenvolvidas** neste arquivo, com número e procedimento — o passo 6 é ensaiar, não escrever.
- **O roteiro de demonstração já existe** (`docs/HA-E-ROTEIRO-DEMO.md` § Parte 2): 45 min com ordem de corte para 30, os 4 blocos do § 7, as 3 respostas de 20 s e o checklist de 10 minutos antes.
- **Cuidado ao rodar a sequência do zero (passo 1)**: `nuke` apaga os volumes. Os 10M levam ~2 min para recarregar e o backfill do ClickHouse ~56 s. **`init/` só roda em volume novo** — é a oportunidade de confirmar que o schema versionado (com a named collection e o `06_pii_access_audit.sql`) sobe limpo do zero, o que nenhuma etapa verificou ainda.
- **Clone limpo já validado na etapa 20** (compose parseia, 13 serviços, entregáveis presentes, DDL sem segredo) — mas **sem `up -d` completo**, porque o ambiente ocupava as portas. O passo 1 desta etapa é o que fecha isso.
- Ambiente: 11 containers, 10.000.000 nas 3 pontas, legado 480/80.000/50.000/15, Dictionary em 100%.

Verificado ao fechar a etapa 16 (segue válido no que não foi substituído acima):
...
| 21 | `audit.sh` `A1.10` passou a falhar **por motivo legítimo** | O limite de 110 linhas existia enquanto o arquivo era só orquestrador — cada linha custa contexto por ser lida a cada etapa. Ao virar também documento de entrega (`docs/METODO-DE-EXECUCAO.md`), ganhou 24 linhas de preâmbulo que a banca lê uma vez e o executor nunca. | Em vez de subir o teto e perder a guarda, o teste passou a medir **só as seções de regra** (`sed` a partir de `## 1. Operação`), que continuam sendo o que pesa no contexto. Teto do arquivo inteiro em 140; o das regras segue < 110. |
| 21 | Duas referências quebraram e **nenhuma era achável por leitura** | `E1.4` apontava para `PREMISSAS-VERIFICADAS.md` na raiz e só falha **depois** do `git mv`; `A7.7`/`01.4` asseriam `wip/trio-challenge` e só falham **depois** do rename. | Ambas corrigidas com o comentário explicando quando mudaram. É a confirmação prática do que o `ESTADO HERDADO` da etapa avisava: mover arquivo sem rodar `audit.sh` e `run_all.sh` depois quebra a suíte em silêncio. |
| 21 | Referências **históricas** mantidas com o nome antigo, de propósito | `LOG-EXECUCAO.md`, este arquivo e as etapas em `concluidas/` citam `CLAUDE.md` e `AUDITORIA-E-REPLANEJAMENTO.md` na raiz. Atualizá-las faria o registro descrever um passado que não existiu. | Só referência **viva** foi varrida (README, `audit.sh`, `run_all.sh`, Seções 2 e 8 do método). O registro histórico fica como está — é evidência, não documentação de uso. |
| 21 | Chave privada encontrada no histórico do git | `desafio-3/backup/minio-certs/private.key` foi commitada em `f8a0e2e` e retirada do versionamento em `17a90dc` — **remover num commit posterior não apaga do histórico**. | **Declarada** em `docs/SEGURANCA-E-GOVERNANCA.md` § 3 em vez de reescrever o histórico: é chave de certificado autoassinado local, que não dá acesso a sistema, dado ou conta. `filter-branch` custaria os 30 commits — o rastro do processo — para remover risco nulo. O critério de quando a decisão seria a oposta (credencial real → rotacionar, reescrever, tratar como incidente) está escrito junto. `.env` nunca foi commitado. |
