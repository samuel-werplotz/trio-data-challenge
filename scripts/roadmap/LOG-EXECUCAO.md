...
## 99 · 2026-08-05
Quebrou: **a sequência do zero reprovou 22 testes.** O ambiente vinha sendo construído incrementalmente havia dias; num volume novo, 4 passos que só existiam na cabeça de quem construiu simplesmente não aconteciam. É o achado central da etapa — e só aparece rodando `nuke`, nunca lendo.
Quebrou: **o RBAC da etapa 19 não existia num ambiente novo.** Eu havia criado `analytics_ro` por SQL ad-hoc e confirmado que persistia em `/var/lib/clickhouse/access/` — mas persistir no volume **não é o mesmo que existir no repositório**. 6 testes reprovaram. Corrigido versionando `init/clickhouse/02_rbac.sql`.
Quebrou: **as stanzas do pgBackRest eram um passo manual não documentado.** `backup-all.sh` pressupunha que existissem; `stanza-create` não aparecia em lugar nenhum do repositório. 6 testes de backup reprovaram e não havia como reproduzir o setup. Criado `desafio-3/backup/init-stanzas.sh`.
Quebrou: **os CAggs nasciam vazios.** `refresh_continuous_aggregate` não roda dentro de bloco de transação, e o `docker-entrypoint-initdb.d` executa cada arquivo numa — então os CALLs de refresh do `04_caggs_policies.sql` são **silenciosamente pulados** no init. Nada no log indica isso.
Quebrou: **o bloat do legado era induzido à mão.** `migration-analysis.md` usa 83,3% de linhas mortas como evidência da recomendação, mas as "5 rodadas de UPDATE sem VACUUM" nunca foram escritas. Criado `desafio-1/scripts/induzir-bloat-legado.sh` — resultado: **90,9% de linhas mortas, 12 MB → 121 MB**.
Quebrou: **o seed do legado falhava com erro de sintaxe, de forma intermitente.** `random() * 1800 || ' days'` produz notação científica quando `random()` cai abaixo de ~1e-4 (`5.005684263093002e-05 days` → `invalid input syntax for type interval`), e o erro derrubava o `DO $$` inteiro, deixando as **4 tabelas do legado vazias**. Defeito pré-existente, em 2 lugares. Corrigido multiplicando o interval em vez de passar por texto.
Quebrou: **`health-check.sh` reportava 4 falhas falsas** — consultava `institutions` (o nome sempre foi `partner_institutions`), cobrava os 3 containers de CDC como obrigatórios (etapa 13 foi DESCARTADA) e chamava o MinIO por `http` quando ele serve TLS. Era a primeira coisa que o avaliador rodaria. Reescrito: agora cobre `sync-worker`, `ref-sync` e `api`, e o CDC virou linha informativa. Resultado: **9/9 verdes, "Ambiente íntegro"**.
Quebrou: **`OPTIMIZE ... FINAL` deduplicou a raw e NÃO as MVs** — e não há merge que resolva. A MV disparou uma vez por versão inserida, então o `AggregatingMergeTree` **contou a duplicata de forma permanente**; ele não tem `_version` para colapsar. Terceira forma da mesma armadilha das etapas 12 e 16. Única saída: reconstruir as 2 MVs a partir da raw já deduplicada (`FINAL`). Incorporado ao `bootstrap.sh`.
Decidido: teste `E0.3` reescrito. Asseria "< 200 segmentos de WAL" e reprovava um ambiente **saudável** recém-carregado — depois do seed o `pg_wal` fica em 3,8 GB, abaixo do `max_wal_size` de 4 GB, e o Postgres não tem motivo para encolher. O risco real não é o número de segmentos, é **arquivamento parado**: passou a exigir `failed_count = 0` **e** uso dentro do `max_wal_size`.
Decidido: criado `scripts/bootstrap.sh` como o fecho de tudo isso. `docker compose up -d` sozinho já entrega o critério nº 1 do PDF; o bootstrap entrega o resto (CAggs, compressão, backfill, RBAC, stanzas, bloat) num comando idempotente. **Configuração que só existe no ambiente vivo é configuração que se perde.**
Notado: a premissa da ficha de que "seed de 10M custa ~20 min" **errou por 7×** — o medido foi **168 s**. Estimativa feita antes de existir gerador, nunca reconferida. Sequência completa do zero: **≈ 6 min**.
Notado: `count()` do ClickHouse fica em 10.003.566 logo após o backfill porque o `sync-worker` ingere durante o seed e o backfill relê essas linhas. `count() FINAL` e `uniqExact(external_id)` dão **10.000.000 exatos** — é o `ReplacingMergeTree` funcionando, não perda nem duplicação real.
